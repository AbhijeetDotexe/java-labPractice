...
try{ 
 bw = new BufferedWriter(new FileWriter(new File("test.txt",true))); 
for(int i=0;i<s.length();i++){ con=(int)s.charAt(i); 
 bw.write(con); 
 } 
 bw.close(); 
 }catch(Exception e){ 
...
while((s1=br.readLine())!=null){
	for(int i=0;i<s1.length();i++){ ch=s1.charAt(i); 
